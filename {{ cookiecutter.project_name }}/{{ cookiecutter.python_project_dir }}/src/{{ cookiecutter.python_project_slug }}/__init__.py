...
# needs ignore for pylance/VSCode
import matplotlib.pyplot as plt  # type: ignore
import numpy as np
from numpy.typing import NDArray

# needs ignore for pylance/VSCode
from {{ cookiecutter.bindings_project_slug }} import gmm as _gmm  # type: ignore
...
def initialize(data: NDArray[np.float64], n_components: int, alpha: float = 1.0) -> GaussianMixtureModel:
    ...
    responsibilities: Likelihood = Likelihood(np.random.dirichlet(n_components * [alpha], data.shape[0]).T)
...
def maximize_v1(gmm: GaussianMixtureModel, responsibilities: Likelihood, data: NDArray[np.float64]) -> None:
...
r = expect(model, data)
    ...
    res = timeit.repeat("maximize_v1(model, r, data)", setup=benchmark, number=n, repeat=repeat)
    print(f"With loops — fastest: {min(res)/n}, slowest: {max(res)/n}, mean: {sum(res)/repeat/n} ")
...
def make_data() -> None:
    ...
    np.save("./data/responsibilities.npy", responsibilities.T)
    np.save("./data/means.npy", gmm.means)
    np.save("./data/weights.npy", gmm.weights)
    np.save("./data/covs.npy", gmm.covs)
